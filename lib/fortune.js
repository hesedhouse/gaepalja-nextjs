...
export function getCoupangRecs(el, missing, breed) {
  const recs = [];
  const 오행추천 = {
    木: [{name:"내추럴코어 유기농 사료",desc:"간 건강에 좋은 유기농 재료",tag:"건강사료",price:"32,900원"},{name:"강아지 우드볼 장난감",desc:"나무 기운을 채워주는 천연 원목 장난감",tag:"장난감",price:"12,500원"}],
    火: [{name:"연어 오메가3 간식",desc:"심장 건강에 좋은 연어 오일 함유",tag:"건강간식",price:"18,900원"},{name:"쿨링 매트 방석",desc:"여름철 체온 조절용 쿨매트",tag:"여름용품",price:"24,500원"}],
    土: [{name:"소화기능 프로바이오틱스",desc:"장 건강을 위한 유산균 보조제",tag:"건강보조",price:"28,000원"},{name:"메모리폼 반려견 침대",desc:"편안한 수면을 위한 정형외과 침대",tag:"침구",price:"45,900원"}],
    金: [{name:"피부모질 오메가 영양제",desc:"피모 건강과 윤기를 위한 영양제",tag:"영양제",price:"22,000원"},{name:"스테인리스 급수기",desc:"위생적인 자동 급수기",tag:"급수용품",price:"19,900원"}],
    水: [{name:"정수 필터 급수대",desc:"깨끗한 물을 위한 3단 필터 급수대",tag:"급수용품",price:"35,900원"},{name:"방수 레인코트",desc:"비 오는 날 산책을 위한 방수 코트",tag:"의류",price:"15,500원"}],
  };
  recs.push(...(오행추천[el]||[]));
  if (missing.includes("水")) recs.push({name:"물놀이 분수 장난감",desc:"부족한 水 기운 보완! 물놀이로 즐거움 UP",tag:"水보완",price:"16,900원"});
  if (missing.includes("木")) recs.push({name:"천연 사슴뿔 뼈간식",desc:"부족한 木 기운 보완! 자연의 에너지 가득",tag:"木보완",price:"13,500원"});
  if (missing.includes("火")) recs.push({name:"LED 야간 산책줄",desc:"부족한 火 기운 보완! 빛나는 산책으로 활력 UP",tag:"火보완",price:"11,900원"});
  if (missing.includes("金")) recs.push({name:"스텐 노즈워크 볼",desc:"부족한 金 기운 보완! 지능 훈련으로 예리함 UP",tag:"金보완",price:"14,500원"});
  if (missing.includes("土")) recs.push({name:"천연 양모 방석",desc:"부족한 土 기운 보완! 대지의 안정감을 선물",tag:"土보완",price:"29,900원"});
  const 체급 = (견종데이터[breed]||견종데이터["기타"]).체급;
  if (체급 === "소형" || 체급 === "초소형") recs.push({name:"소형견 치석제거 덴탈껌",desc:`${체급} 맞춤! 구강 건강 관리`,tag:"구강관리",price:"9,900원"});
  else recs.push({name:"대형견 관절 영양제",desc:`${체급} 맞춤! 관절 보호 글루코사민`,tag:"관절관리",price:"33,000원"});
